> **Archivo histórico.** Esta carpeta no es el schema vigente. La fuente de verdad es `supabase/migrations/` (baseline + 100+ migraciones). No ejecutes estos SQL sobre `b2bconecta-db` ni `b2bconecta-db-dev`. Desarrollo actual: [`docs/GETTING_STARTED.md`](../../GETTING_STARTED.md) y [`docs/ARCHITECTURE.md`](../../ARCHITECTURE.md).
>
> `schema.sql` es un snapshot de la baseline. Los `upgrade_*.sql` eran parches one-shot para bases MotoConecta antiguas (hoy cubiertos por migraciones).
...
Documento de origen del modelo (nombres C1–C4 / E2–E4). El producto en producción es **B2B Conecta**; el package Flutter sigue siendo `motolink_pro_app`.
