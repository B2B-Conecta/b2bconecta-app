...
class SupabaseService {
  ...
  static Future<String> createSupportTicket({
    ...
    final res = await _client.rpc('create_support_ticket', params: params);
    return res?.toString() ?? '';
  }
  ...
  static Future<List<ImporterCarrierModel>> listMyImporterCarriers() async {
    final response = await _client.rpc('list_my_importer_carriers');
  ...
  static Future<({
    ...
    if (res is! Map) {
      throw StateError('Respuesta inválida al consultar datos del transportista.');
    }
  ...
  static Future<void> importerSubmitFleteFactura({
    ...
    final m = Map<String, dynamic>.from(row);
    if (m['importador_id']?.toString() != uid) {
      throw StateError('No autorizado a adjuntar factura de flete en este pedido.');
    }
    final st = m['status']?.toString();
    if (st != TransactionRequestStatus.pendiente &&
        st != TransactionRequestStatus.enPreparacion &&
