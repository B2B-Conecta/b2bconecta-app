-- A2: destino de entrega por pedido + declaración de pago en cualquier fase operativa del ciclo.

alter table public.transaction_requests
  add column if not exists destino_entrega_usa_perfil boolean not null default true,
  add column if not exists destino_entrega_texto text,
  add column if not exists destino_entrega_maps_url text;

comment on column public.transaction_requests.destino_entrega_usa_perfil is
  'Si true, la mercancía se entrega en la dirección fiscal del perfil del aliado.';
comment on column public.transaction_requests.destino_entrega_texto is
  'Si usa_perfil es false, dirección o referencia del destino indicada al solicitar el pedido.';
comment on column public.transaction_requests.destino_entrega_maps_url is
  'Opcional: enlace a Google Maps u otro mapa para el destino alternativo.';

-- Multietapa: permite comprobante / declaración si el pedido no está rechazado.
...
begin
  ...
  if n = 0 then
    raise exception
      'No se pudo registrar la solicitud. Verifique factura MotoLink, que el pedido siga activo '
      '(no rechazado) y que pueda reintentar si el pago fue rechazado.';
  end if;
end;
$$;
...
begin
  ...
  if n = 0 then
    raise exception
      'No se pudo registrar el comprobante. Verifique que exista la factura MotoLink, que el pedido '
      'no esté rechazado y que el pago no esté ya aprobado.';
  end if;
end;
$$;
...
begin
  ...
  if n = 0 then
    raise exception
      'No se pudo registrar la declaración. Verifique factura MotoLink, que el pedido siga activo '
      '(no rechazado) y que pueda reenviar si el pago fue rechazado.';
  end if;
end;
$$;
