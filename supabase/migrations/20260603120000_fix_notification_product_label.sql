-- Los triggers de notificación referenciaban new.product_name (no existe en
-- transaction_requests). Etiqueta vía join a products.product_id.

create or replace function public.mc_tr_product_label (p_row public.transaction_requests)
returns text
language plpgsql
stable
security definer
...
declare
  v_name text;
begin
  if p_row.product_id is not null then
    select nullif(trim(pr.name), '')
      into v_name
    from public.products pr
    where pr.id = p_row.product_id;
  end if;
  ...
  return 'Pedido ' || left(p_row.id::text, 8);
end;
$$;

-- ---------------------------------------------------------------------------
-- Pago aprobado tras moroso (dispara al confirmar comprobante — importador)
-- ---------------------------------------------------------------------------
...
begin
  ...
  perform public.mc_insert_notification (
    ...
    format(
      'Confirmó el pago del pedido «%s». El estado moroso quedó regularizado.',
      v_product
    ),
    'pago',
    v_anchor
