-- E1.2: atribución de pedido a campaña promocional + RPC importador.
...
create index if not exists transaction_requests_promo_campaign_idx
  on public.transaction_requests (promo_campaign_id)
  where promo_campaign_id is not null;

comment on column public.transaction_requests.promo_campaign_id is
  'Campaña E1.2 bajo la cual el aliado solicitó el pedido (CTA promocional).';

-- Campañas activas del importador autenticado.
...
grant execute on function public.get_active_promo_campaigns_for_importador () to authenticated;

-- Checkout: opcional p_promo_by_importador {"<importador_uuid>": "<campaign_uuid>", ...}
