-- Referencia legal del mayorista (contacto legal / compañía) + requisito en solicitud de ingreso.

alter table public.profiles
  add column if not exists legal_reference text;

comment on column public.profiles.legal_reference is
  'Importador: contacto legal de la empresa (nombre, cargo, datos de contacto).';
...
begin
  ...
  if not public.profile_importador_profile_complete (v_uid) then
    raise exception
      'Complete nombre comercial, RIF, dirección fiscal, enlace de Google Maps y referencia legal.';
  end if;
