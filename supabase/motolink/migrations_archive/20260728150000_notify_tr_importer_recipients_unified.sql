-- Refuerzo (tras 20260728140000):
-- 1) notify_tr_importer_recipients: siempre intenta sub_orders primero; si 0 filas, owner_id.
--    Así no depende solo de is_master_order (datos viejos / migración no aplicada).
-- 2) Vuelve a publicar las funciones de notificación que usan el helper, por si en remoto
--    sigue activa la versión previa a 20260728140000.
...
begin
  ...
  if coalesce(n, 0) = 0 and p_owner_id is not null then
    insert into public.notifications (user_id, title, body, type, related_id)
...
comment on function public.notify_tr_importer_recipients is
  'In-app: primero notifica por sub_orders; si 0 filas y hay owner_id, notificación 1:1.';
